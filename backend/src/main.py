...
class GetAuthorizationReturn(BaseModel):
    ...
    user: typing.Optional[models.User.Private] = None
    scope_ids: typing.Optional[set[client.ScopeTypes.id]] = None
    auth_credential: typing.Optional[models.AuthCredentialModel] = None
...
async def get_authorization(
    ...
    AuthCredentialModel = models.AUTH_CREDENTIAL_TYPE_TO_MODEL[auth_type]

    # validate the jwt claims
    if not AuthCredentialModel.validate_jwt_claims(payload):
        return GetAuthorizationReturn(exception='missing_required_claims')

    auth_credential_from_jwt: models.AuthCredentialModel = AuthCredentialModel(
        **AuthCredentialModel.decode(payload))
    ...
    user_private: models.User.Private = None
    scope_ids: set[client.ScopeTypes.id] = set()

    if issubclass(AuthCredentialModel, models.Table):

        with Session(c.db_engine) as session:
            auth_credential_from_db = await AuthCredentialModel.get_one_by_id(
            ...
            user: models.User.UserModel = auth_credential_from_db.user
            if user == None:
                return GetAuthorizationReturn(exception='user_not_found')

            if AuthCredentialModel.auth_type == 'access_token':
                scope_ids = c.user_role_id_scope_ids[user.user_role_id]
            elif AuthCredentialModel.auth_type == 'api_key':
                scope_ids = await auth_credential_from_db.get_scope_ids()

            user_private = models.User.Private.model_validate(user)
...
class GetAuthBaseReturn(BaseModel):
    user: typing.Optional[models.User.Private]
...
async def authenticate_user_with_username_and_password(form_data: typing.Annotated[OAuth2PasswordRequestForm, Depends()]) -> models.User.UserModel:
    with Session(c.db_engine) as session:
        user = await models.User.UserModel.authenticate(
...
async def post_token(
    user: typing.Annotated[models.User.UserModel, Depends(authenticate_user_with_username_and_password)],
    response: Response,
    stay_signed_in: bool = Form(c.authentication['stay_signed_in_default'])
) -> TokenResponse:
    with Session(c.db_engine) as session:
        user_access_token = await models.UserAccessToken.UserAccessTokenModel.api_post(
            session=session, c=c, authorized_user_id=user._id, admin=False, create_model=models.UserAccessToken.AdminCreate(
...
async def post_login_password(
    ...
    with Session(c.db_engine) as session:
        ...
        return LoginWithPasswordResponse(
            auth=GetAuthBaseReturn(
                user=models.User.Private.model_validate(user),
                scope_ids=set(
...
async def post_login_magic_link(
    ...
    with Session(c.db_engine) as session:
        user_access_token = await models.UserAccessToken.UserAccessTokenModel.api_post(
            session=session, c=c, authorized_user_id=authorization._user_id, admin=False, create_model=models.UserAccessToken.AdminCreate(
        ...
        return LoginWithMagicLinkResponse(
            auth=GetAuthBaseReturn(
                user=models.User.Private.model_validate(authorization.user),
                scope_ids=set(
...
async def post_login_otp(
    ...
    with Session(c.db_engine) as session:
        ...
        if 'email' in model.model_fields_set:
            user = session.exec(select(models.User.UserModel).where(
                models.User.UserModel.email == model.email)).one_or_none()
        elif 'phone_number' in model.model_fields_set:
            user = session.exec(select(models.User.UserModel).where(
                models.User.UserModel.phone_number == model.phone_number)).one_or_none()
        ...
        hashed_code = models.OTP.OTPModel.hash_code(model.code)
        otp = session.exec(select(models.OTP.OTPModel).where(models.OTP.OTPModel.user_id == user.id).where(
            models.OTP.OTPModel.hashed_code == hashed_code)).one_or_none()
...
async def post_login_google(request_token: LoginWithGoogleRequest, response: Response) -> LoginWithGoogleResponse:
    ...
    with Session(c.db_engine) as session:
        ...
        user = session.exec(select(models.User.UserModel).where(
            models.User.UserModel.email == email)).one_or_none()
        if not user:
            user = await models.User.UserModel.api_post(session=session, c=c, authorized_user_id=None, create_model=models.User.AdminCreate(email=email, user_role_id=c.user_role_name_mapping['user']))
...
async def post_signup(response: Response, token: str = Query(None)) -> PostSignUpResponse:
    ...
    with Session(c.db_engine) as session:
        sign_up: models.SignUp.SignUpModel = authorization.auth_credential

        user_create_admin = models.User.AdminCreate(
            email=sign_up.email, user_role_id=c.user_role_name_mapping['user'])

        user = await models.User.UserModel.api_post(session=session, c=c, authorized_user_id=None,
...
@ auth_router.post('/request-signup/', responses={status.HTTP_409_CONFLICT: {"description": models.User.UserModel.already_exists_message(), 'model': DetailOnlyResponse}})
async def post_request_sign_up(
    ...
    with Session(c.db_engine) as session:
        ...
        if user:
            c.send_email(
                email, 'Someone is trying to sign up with your email', '')

        # send them a sign up link
        else:
            sign_up = models.SignUp.SignUpModel.create(
                models.SignUp.AdminCreate(
            ...
            c.send_email(
                email, 'Sign Up', 'Click to sign up: {}'.format(url))
    ...
    with Session(c.db_engine) as session:
        ...
        if user:
            user_access_token = await models.UserAccessToken.UserAccessTokenModel.api_post(
                ...
                create_model=models.UserAccessToken.AdminCreate(user_id=user._id, lifespan=c.authentication['expiry_timedeltas']['request_magic_link']))
...
async def post_send_otp(model: RequestOTPRequest, background_tasks: BackgroundTasks) -> DetailOnlyResponse:
    ...
    if 'email' in model.model_fields_set or 'phone_number' in model.model_fields_set:
        code = models.OTP.OTPModel.generate_code()
...
async def logout(response: Response, authorization: typing.Annotated[GetAuthorizationReturn, Depends(
    ...
    if authorization.auth_credential:
        if authorization.auth_credential.type == 'access_token':
            with Session(c.db_engine) as session:
                await models.UserAccessToken.UserAccessTokenModel.api_delete(session=session, c=c, authorized_user_id=authorization._user_id, id=authorization.auth_credential.id, admin=False)
...
user_router = APIRouter(
    prefix='/users', tags=[models.User.UserModel._ROUTER_TAG])
user_admin_router = APIRouter(
    prefix='/admin/users', tags=[models.User.UserModel._ROUTER_TAG, 'Admin'])


@ user_router.get('/me/', responses=models.User.UserModel.get_responses())
async def get_user_me(
    authorization: typing.Annotated[GetAuthorizationReturn, Depends(
        make_get_auth_dependency())]
) -> models.User.Private:
    return models.User.Private.model_validate(authorization.user)


@ user_router.get('/{user_id}/', responses=models.User.UserModel.get_responses())
async def get_user_by_id(
    ...
    with Session(c.db_engine) as session:
        return models.User.Public.model_validate(await models.User.UserModel.api_get(session=session, c=c, authorized_user_id=authorization._user_id, id=user_id, admin=False))


@ user_admin_router.get('/{user_id}/', responses=models.User.UserModel.get_responses())
async def get_user_by_id_admin(
    ...
    with Session(c.db_engine) as session:
        a = await models.User.UserModel.api_get(session=session, c=c, authorized_user_id=authorization._user_id, id=user_id, admin=True)


@ user_admin_router.post('/', responses=models.User.UserModel.post_responses())
async def post_user_admin(
    ...
    with Session(c.db_engine) as session:
        return await models.User.UserModel.api_post(session=session, c=c, authorized_user_id=authorization._user_id, admin=True, create_model=user_create_admin)


@ user_router.patch('/me/', responses=models.User.UserModel.patch_responses())
async def patch_self(
    ...
    with Session(c.db_engine) as session:
        return models.User.Private.model_validate(await models.User.api_patch(session=session, c=c, authorized_user_id=authorization._user_id, id=authorization._user_id, admin=False, update_model=models.UserUpdateAdmin(**user_update.model_dump(exclude_unset=True))))


@ user_admin_router.patch('/{user_id}/', responses=models.User.UserModel.patch_responses())
async def patch_user_admin(
    user_id: models.UserTypes.id,
    user_update_admin: models.User.AdminUpdate,
    authorization: typing.Annotated[GetAuthorizationReturn, Depends(
        make_get_auth_dependency(required_scopes={'admin'}))]
) -> models.User.UserModel:
    with Session(c.db_engine) as session:
        return await models.User.UserModel.api_patch(session=session, c=c, authorized_user_id=authorization._user_id, id=user_id, admin=True, update_model=user_update_admin)


@ user_router.delete('/me/', responses=models.User.UserModel.delete_responses(), status_code=status.HTTP_204_NO_CONTENT)
async def delete_self(
    authorization: typing.Annotated[GetAuthorizationReturn, Depends(
        make_get_auth_dependency())]
):
    with Session(c.db_engine) as session:
        return await models.User.UserModel.api_delete(session=session, c=c, authorized_user_id=authorization._user_id, id=authorization._user_id, admin=False)


@ user_admin_router.delete('/{user_id}/', responses=models.User.UserModel.delete_responses(), status_code=status.HTTP_204_NO_CONTENT)
async def delete_user_admin(
    ...
    with Session(c.db_engine) as session:
        return await models.User.UserModel.api_delete(session=session, c=c, authorized_user_id=authorization._user_id, id=user_id, admin=True)


@ user_router.get('/', responses=models.User.UserModel.post_responses())
async def get_users(
    pagination: typing.Annotated[models.Pagination, Depends(get_pagination())]
) -> list[models.User.Public]:

    with Session(c.db_engine) as session:
        query = select(models.User).where(
            models.User.UserModel.username != None)
        query = models.build_pagination(query, pagination)
        users = session.exec(query).all()
        return [models.User.Public.model_validate(user) for user in users]
...
async def get_users_admin(
    authorization: typing.Annotated[GetAuthorizationReturn, Depends(
        make_get_auth_dependency(required_scopes={'admin'}))],
    pagination: typing.Annotated[models.Pagination, Depends(get_pagination())]
) -> list[models.User.UserModel]:

    with Session(c.db_engine) as session:
        query = select(models.User.UserModel)
        query = models.build_pagination(query, pagination)
        users = session.exec(query).all()
        return users
...
async def get_user_username_available(username: models.UserTypes.username):
    with Session(c.db_engine) as session:
        return await models.User.UserModel.api_get_is_username_available(session, username)
...
user_access_token_router = APIRouter(
    prefix='/user-access-tokens', tags=[models.UserAccessToken.UserAccessTokenModel._ROUTER_TAG])
user_access_token_admin_router = APIRouter(
    prefix='/admin/user-access-tokens', tags=[models.UserAccessToken.UserAccessTokenModel._ROUTER_TAG, 'Admin'])


@ user_access_token_router.get('/{user_access_token_id}/', responses=models.UserAccessToken.UserAccessTokenModel.get_responses())
async def get_user_access_token_by_id(
    user_access_token_id: models.UserAccessTokenTypes.id,
    authorization: typing.Annotated[GetAuthorizationReturn, Depends(
        make_get_auth_dependency())]
) -> models.UserAccessToken.UserAccessTokenModel:
    with Session(c.db_engine) as session:
        return await models.UserAccessToken.UserAccessTokenModel.api_get(session=session, c=c, authorized_user_id=authorization._user_id, id=user_access_token_id, admin=False)


@ user_access_token_admin_router.get('/{user_access_token_id}/', responses=models.UserAccessToken.UserAccessTokenModel.get_responses())
async def get_user_access_token_by_id_admin(
    ...
    with Session(c.db_engine) as session:
        return await models.UserAccessToken.UserAccessTokenModel.api_get(session=session, c=c, authorized_user_id=authorization._user_id, id=user_access_token_id, admin=True)


@ user_access_token_admin_router.post('/', responses=models.UserAccessToken.UserAccessTokenModel.post_responses())
async def post_user_access_token_admin(
    user_access_token_create_admin: models.UserAccessToken.AdminCreate,
    authorization: typing.Annotated[GetAuthorizationReturn, Depends(
        make_get_auth_dependency(required_scopes={'admin'}))]
) -> models.UserAccessToken.UserAccessTokenModel:
    with Session(c.db_engine) as session:
        return await models.UserAccessToken.UserAccessTokenModel.api_post(session=session, c=c, authorized_user_id=authorization._user_id, admin=True, create_model=user_access_token_create_admin)


@ user_access_token_router.delete('/{user_access_token_id}/', responses=models.UserAccessToken.UserAccessTokenModel.delete_responses(), status_code=status.HTTP_204_NO_CONTENT)
async def delete_user_access_token(
    ...
    with Session(c.db_engine) as session:
        return await models.UserAccessToken.UserAccessTokenModel.api_delete(session=session, c=c, authorized_user_id=authorization._user_id, id=user_access_token_id, admin=False)


@ user_access_token_admin_router.delete('/{user_access_token_id}/', responses=models.UserAccessToken.UserAccessTokenModel.delete_responses(), status_code=status.HTTP_204_NO_CONTENT)
async def delete_user_access_token_admin(
    ...
    with Session(c.db_engine) as session:
        return await models.UserAccessToken.UserAccessTokenModel.api_delete(session=session, c=c, authorized_user_id=authorization._user_id, id=user_access_token_id, admin=True)
...
@ user_access_token_router.get('/', tags=[models.User.UserModel._ROUTER_TAG], responses={status.HTTP_404_NOT_FOUND: {"description": models.User.UserModel.not_found_message(), 'model': NotFoundResponse}, WrongUserPermissionUserAccessToken.status_code: {'description': WrongUserPermissionUserAccessToken.detail, 'model': DetailOnlyResponse}})
async def get_user_access_tokens(
    authorization: typing.Annotated[GetAuthorizationReturn, Depends(
        make_get_auth_dependency())],
    pagination: models.Pagination = Depends(user_access_token_pagination)

) -> list[models.UserAccessToken.UserAccessTokenModel]:
    with Session(c.db_engine) as session:
        query = select(models.UserAccessToken.UserAccessTokenModel).where(
            models.UserAccessToken.UserAccessTokenModel.user_id == authorization._user_id)
        query = models.build_pagination(query, pagination)
        user_access_tokens = session.exec(query).all()
        return user_access_tokens


@ user_access_token_admin_router.get('/users/{user_id}/', tags=[models.User.UserModel._ROUTER_TAG], responses={status.HTTP_404_NOT_FOUND: {"description": models.User.UserModel.not_found_message(), 'model': NotFoundResponse}})
async def get_user_access_tokens_admin(
    ...
    with Session(c.db_engine) as session:
        ...
        query = select(models.UserAccessToken.UserAccessTokenModel).where(
            models.UserAccessToken.UserAccessTokenModel.user_id == user_id)
        query = models.build_pagination(query, pagination)
        user_access_tokens = session.exec(query).all()
        return user_access_tokens
...
async def get_user_access_tokens_count(
    ...
    with Session(c.db_engine) as session:
        query = select(func.count()).select_from(models.UserAccessToken.UserAccessTokenModel).where(
            models.UserAccessToken.UserAccessTokenModel.user_id == authorization._user_id)
...
api_key_router = APIRouter(
    prefix='/api-keys', tags=[models.ApiKey.ApiKeyModel._ROUTER_TAG])
api_key_admin_router = APIRouter(
    prefix='/admin/api-keys', tags=[models.ApiKey.ApiKeyModel._ROUTER_TAG, 'Admin'])


@ api_key_router.get('/{api_key_id}/', responses=models.ApiKey.ApiKeyModel.get_responses())
async def get_api_key_by_id(
    api_key_id: models.ApiKeyTypes.id,
    authorization: typing.Annotated[GetAuthorizationReturn, Depends(
        make_get_auth_dependency())]
) -> models.ApiKey.Private:
    with Session(c.db_engine) as session:
        return models.ApiKey.Private.model_validate(await models.ApiKey.ApiKeyModel.api_get(session=session, c=c, authorized_user_id=authorization.user.id, id=api_key_id, admin=False))


@ api_key_admin_router.get('/{api_key_id}/', responses=models.ApiKey.ApiKeyModel.get_responses())
async def get_api_key_by_id_admin(
    api_key_id: models.ApiKeyTypes.id,
    authorization: typing.Annotated[GetAuthorizationReturn, Depends(
        make_get_auth_dependency(required_scopes={'admin'}))]
) -> models.ApiKey.ApiKeyModel:
    with Session(c.db_engine) as session:
        return await models.ApiKey.ApiKeyModel.api_get(session=session, c=c, authorized_user_id=authorization.user.id, id=api_key_id, admin=True)


@ api_key_router.post('/', responses=models.ApiKey.ApiKeyModel.post_responses())
async def post_api_key_to_user(
    api_key_create: models.ApiKey.Create,
    authorization: typing.Annotated[GetAuthorizationReturn, Depends(
        make_get_auth_dependency())]
) -> models.ApiKey.Private:

    with Session(c.db_engine) as session:
        return models.ApiKey.Private.from_api_key(await models.ApiKey.ApiKeyModel.api_post(session=session, c=c, authorized_user_id=authorization._user_id, admin=False, create_model=models.ApiKey.AdminCreate(
            **api_key_create.model_dump(exclude_unset=True), user_id=authorization._user_id)))


@ api_key_admin_router.post('/', responses=models.ApiKey.ApiKeyModel.post_responses())
async def post_api_key_to_user_admin(
    api_key_create_admin: models.ApiKey.AdminCreate,
    authorization: typing.Annotated[GetAuthorizationReturn, Depends(
        make_get_auth_dependency(required_scopes={'admin'}))]
) -> models.ApiKey.ApiKeyModel:
    with Session(c.db_engine) as session:
        return await models.ApiKey.ApiKeyModel.api_post(session=session, c=c, authorized_user_id=authorization.user.id, admin=True, create_model=models.ApiKey.AdminCreate(**api_key_create_admin.model_dump(exclude_unset=True)))


@ api_key_router.patch('/{api_key_id}/', responses=models.ApiKey.ApiKeyModel.patch_responses())
async def patch_api_key(
    api_key_id: models.ApiKeyTypes.id,
    api_key_update: models.ApiKey.Update,
    authorization: typing.Annotated[GetAuthorizationReturn, Depends(
        make_get_auth_dependency())]
) -> models.ApiKey.Private:
    with Session(c.db_engine) as session:
        return models.ApiKey.Private.from_api_key(await models.ApiKey.ApiKeyModel.api_patch(session=session, c=c, authorized_user_id=authorization.user.id, id=api_key_id, admin=False, update_model=models.ApiKey.AdminUpdate(**api_key_update.model_dump(exclude_unset=True))))


@ api_key_admin_router.patch('/{api_key_id}/', responses=models.ApiKey.ApiKeyModel.patch_responses())
async def patch_api_key_admin(
    api_key_id: models.ApiKeyTypes.id,
    api_key_update_admin: models.ApiKey.AdminUpdate,
    authorization: typing.Annotated[GetAuthorizationReturn, Depends(
        make_get_auth_dependency(required_scopes={'admin'}))]
) -> models.ApiKey.ApiKeyModel:
    with Session(c.db_engine) as session:
        return await models.ApiKey.ApiKeyModel.api_patch(session=session, c=c, authorized_user_id=authorization.user.id, id=api_key_id, admin=True, update_model=models.ApiKey.AdminUpdate(**api_key_update_admin.model_dump(exclude_unset=True)))


@ api_key_router.delete('/{api_key_id}/', responses=models.ApiKey.ApiKeyModel.delete_responses(), status_code=status.HTTP_204_NO_CONTENT)
async def delete_api_key(
    ...
    with Session(c.db_engine) as session:
        return await models.ApiKey.ApiKeyModel.api_delete(session=session, c=c, authorized_user_id=authorization.user.id, id=api_key_id, admin=False)


@ api_key_admin_router.delete('/{api_key_id}/', responses=models.ApiKey.ApiKeyModel.delete_responses(), status_code=status.HTTP_204_NO_CONTENT)
async def delete_api_key_admin(
    ...
    with Session(c.db_engine) as session:
        return await models.ApiKey.ApiKeyModel.api_delete(session=session, c=c, authorized_user_id=authorization.user.id, id=api_key_id, admin=True)
...
@ api_key_router.get('/{api_key_id}/generate-jwt/', responses={status.HTTP_404_NOT_FOUND: {"description": models.ApiKey.ApiKeyModel.not_found_message(), 'model': NotFoundResponse}})
async def get_api_key_jwt(
    ...
    with Session(c.db_engine) as session:
        api_key = await models.ApiKey.ApiKeyModel.api_get(session=session, c=c, authorized_user_id=authorization.user.id, id=api_key_id, admin=False)
...
def get_api_keys_query_params(
    ...
    order_bys: typing.Annotated[list[models.OrderBy[models.ApiKeyTypes.order_by]], Depends(
        models.ApiKey.ApiKeyModel.get_order_by_depends())]
):
...
) -> list[models.ApiKey.Private]:
    with Session(c.db_engine) as session:

        query = select(models.ApiKey).options(selectinload(models.ApiKey.ApiKeyModel.api_key_scopes)).where(
            models.ApiKey.ApiKeyModel.user_id == authorization._user_id)
        query = models.build_pagination(
            query, get_api_keys_query_params.pagination)
        query = models.ApiKey.ApiKeyModel._build_order_by(
            query, get_api_keys_query_params.order_bys)

        user_api_keys = session.exec(query).all()
        return [models.ApiKey.Private.from_api_key(api_key) for api_key in user_api_keys]
...
@ api_key_admin_router.get('/users/{user_id}/', tags=[models.User.UserModel._ROUTER_TAG])
async def get_user_api_keys_admin(
    ...
    get_api_keys_query_params: GetApiKeysQueryParamsReturn = Depends(
        get_api_keys_query_params)
) -> list[models.ApiKey.ApiKeyModel]:
    with Session(c.db_engine) as session:

        query = select(models.ApiKey).where(
            models.ApiKey.ApiKeyModel.user_id == user_id)
        query = models.build_pagination(
            query, get_api_keys_query_params.pagination)
        query = models.ApiKey.ApiKeyModel._build_order_by(
            query, get_api_keys_query_params.order_bys)
...
@ api_key_router.get('/details/available/', responses={status.HTTP_409_CONFLICT: {"description": models.ApiKey.ApiKeyModel.already_exists_message(), 'model': DetailOnlyResponse}})
async def get_api_key_available(
    authorization: typing.Annotated[GetAuthorizationReturn, Depends(
        make_get_auth_dependency())],
    api_key_available: models.ApiKey.Available = Depends(),
):
    with Session(c.db_engine) as session:
        await models.ApiKey.ApiKeyModel.api_get_is_available(session, models.ApiKey.AdminAvailable(
            **api_key_available.model_dump(exclude_unset=True), user_id=authorization.user.id
        ))


@ api_key_admin_router.get('/details/available/', responses={status.HTTP_409_CONFLICT: {"description": models.ApiKey.ApiKeyModel.already_exists_message(), 'model': DetailOnlyResponse}})
async def get_api_key_available_admin(
    authorization: typing.Annotated[GetAuthorizationReturn, Depends(
        make_get_auth_dependency(required_scopes={'admin'}))],
    api_key_available_admin: models.ApiKey.AdminAvailable = Depends(),
):
    with Session(c.db_engine) as session:
        await models.ApiKey.ApiKeyModel.api_get_is_available(session, api_key_available_admin)
...
api_key_scope_router = APIRouter(
    prefix='/api-key-scopes', tags=[models.ApiKeyScope.ApiKeyScopeModel._ROUTER_TAG])
api_key_scope_admin_router = APIRouter(
    prefix='/admin/api-key-scopes', tags=[models.ApiKeyScope.ApiKeyScopeModel._ROUTER_TAG, 'Admin'])


@ api_key_scope_router.post('/api-keys/{api_key_id}/scopes/{scope_id}/',
                            responses={status.HTTP_404_NOT_FOUND: {
                                "description": models.ApiKey.ApiKeyModel.not_found_message(), 'model': NotFoundResponse}},
                            )
async def add_scope_to_api_key(
    ...
    with Session(c.db_engine) as session:
        return await models.ApiKeyScope.ApiKeyScopeModel.api_post(session=session, c=c, authorized_user_id=authorization.user.id, create_model=models.ApiKeyScope.AdminCreate(api_key_id=api_key_id, scope_id=scope_id), admin=False)
...
async def add_scope_to_api_key_admin(
    ...
    with Session(c.db_engine) as session:
        return await models.ApiKeyScope.ApiKeyScopeModel.api_post(session=session, c=c, authorized_user_id=authorization.user.id, create_model=models.ApiKeyScope.AdminCreate(api_key_id=api_key_id, scope_id=scope_id), admin=True)
...
async def remove_scope_from_api_key(
    ...
    with Session(c.db_engine) as session:
        return await models.ApiKeyScope.ApiKeyScopeModel.api_delete(session=session, c=c, authorized_user_id=authorization.user.id, id=models.ApiKeyScope.IdBase(api_key_id=api_key_id, scope_id=scope_id)._id, admin=False)
...
async def remove_scope_from_api_key_admin(
    ...
    with Session(c.db_engine) as session:
        return await models.ApiKeyScope.ApiKeyScopeModel.api_delete(session=session, c=c, authorized_user_id=authorization.user.id, id=models.ApiKeyScope.IdBase(api_key_id=api_key_id, scope_id=scope_id)._id, admin=True)
...
gallery_router = APIRouter(
    prefix='/galleries', tags=[models.Gallery.GalleryModel._ROUTER_TAG])
gallery_admin_router = APIRouter(prefix='/admin/galleries', tags=[
    models.Gallery.GalleryModel._ROUTER_TAG, 'Admin'])


@ gallery_router.get('/{gallery_id}/', responses=models.Gallery.GalleryModel.get_responses())
async def get_gallery_by_id(
    gallery_id: models.GalleryTypes.id,
    authorization: typing.Annotated[GetAuthorizationReturn, Depends(
        make_get_auth_dependency(raise_exceptions=False))]
) -> models.Gallery.Public:
    with Session(c.db_engine) as session:
        return models.Gallery.Public.model_validate(await models.Gallery.GalleryModel.api_get(session=session, c=c, authorized_user_id=authorization._user_id, id=gallery_id, admin=False))


@ gallery_admin_router.get('/{gallery_id}/', responses=models.Gallery.GalleryModel.get_responses())
async def get_gallery_by_id_admin(
    ...
    with Session(c.db_engine) as session:
        return await models.Gallery.GalleryModel.api_get(session=session, c=c, authorized_user_id=authorization._user_id, id=gallery_id, admin=True)


@ gallery_router.post('/', responses=models.Gallery.GalleryModel.post_responses())
async def post_gallery(
    gallery_create: models.Gallery.Create,
    authorization: typing.Annotated[GetAuthorizationReturn, Depends(
        make_get_auth_dependency())]
) -> models.Gallery.Private:
    with Session(c.db_engine) as session:
        return models.Gallery.Private.model_validate(await models.Gallery.GalleryModel.api_post(session=session, c=c, authorized_user_id=authorization._user_id, admin=False, create_model=models.Gallery.AdminCreate(**gallery_create.model_dump(exclude_unset=True), user_id=authorization._user_id)))


@ gallery_admin_router.post('/', responses=models.Gallery.GalleryModel.post_responses())
async def post_gallery_admin(
    gallery_create_admin: models.Gallery.AdminCreate,
    authorization: typing.Annotated[GetAuthorizationReturn, Depends(
        make_get_auth_dependency(required_scopes={'admin'}))]
) -> models.Gallery.GalleryModel:
    with Session(c.db_engine) as session:
        return await models.Gallery.GalleryModel.api_post(session=session, c=c, authorized_user_id=authorization._user_id, admin=True, create_model=models.Gallery.AdminCreate(**gallery_create_admin.model_dump(exclude_unset=True)))


@ gallery_router.patch('/{gallery_id}/', responses=models.Gallery.GalleryModel.patch_responses())
async def patch_gallery(
    gallery_id: models.GalleryTypes.id,
    gallery_update: models.Gallery.Update,
    authorization: typing.Annotated[GetAuthorizationReturn, Depends(
        make_get_auth_dependency())]
) -> models.Gallery.Private:
    with Session(c.db_engine) as session:
        return models.Gallery.Private.model_validate(await models.Gallery.GalleryModel.api_patch(session=session, c=c, authorized_user_id=authorization._user_id, id=gallery_id, admin=False, update_model=models.Gallery.AdminUpdate(**gallery_update.model_dump(exclude_unset=True))))


@ gallery_admin_router.patch('/{gallery_id}/', responses=models.Gallery.GalleryModel.patch_responses())
async def patch_gallery_admin(
    gallery_id: models.GalleryTypes.id,
    gallery_update_admin: models.Gallery.AdminUpdate,
    authorization: typing.Annotated[GetAuthorizationReturn, Depends(
        make_get_auth_dependency(required_scopes={'admin'}))]
) -> models.Gallery.GalleryModel:
    with Session(c.db_engine) as session:
        return await models.Gallery.GalleryModel.api_patch(session=session, c=c, authorized_user_id=authorization._user_id, id=gallery_id, admin=True, update_model=models.Gallery.AdminUpdate(**gallery_update_admin.model_dump(exclude_unset=True)))


@ gallery_router.delete('/{gallery_id}/', responses=models.Gallery.GalleryModel.delete_responses(), status_code=status.HTTP_204_NO_CONTENT)
async def delete_gallery(
    ...
    with Session(c.db_engine) as session:
        return await models.Gallery.GalleryModel.api_delete(session=session, c=c, authorized_user_id=authorization._user_id, id=gallery_id, admin=False)


@ gallery_admin_router.delete('/{gallery_id}/', responses=models.Gallery.GalleryModel.delete_responses(), status_code=status.HTTP_204_NO_CONTENT)
async def delete_gallery_admin(
    ...
    with Session(c.db_engine) as session:
        return await models.Gallery.GalleryModel.api_delete(session=session, c=c, authorized_user_id=authorization._user_id, id=gallery_id, admin=True)
...
async def get_gallery_available(
    authorization: typing.Annotated[GetAuthorizationReturn, Depends(
        make_get_auth_dependency())],
    gallery_available: models.Gallery.Available = Depends(),
):
    with Session(c.db_engine) as session:
        await models.Gallery.GalleryModel.api_get_is_available(session, models.Gallery.AdminAvailable(
...
async def get_gallery_available_admin(
    authorization: typing.Annotated[GetAuthorizationReturn, Depends(
        make_get_auth_dependency(required_scopes={'admin'}))],
    gallery_available_admin: models.Gallery.AdminAvailable = Depends(),
):
    with Session(c.db_engine) as session:
        await models.Gallery.GalleryModel.api_get_is_available(session, gallery_available_admin)
...
) -> list[models.Gallery.Private]:

    with Session(c.db_engine) as session:
        galleries = session.exec(select(models.Gallery.GalleryModel).where(
            models.Gallery.GalleryModel.user_id == authorization._user_id).offset(offset).limit(limit)).all()
        return [models.Gallery.Private.model_validate(gallery) for gallery in galleries]
...
@ gallery_admin_router.get('/users/{user_id}', tags=[models.User.UserModel._ROUTER_TAG])
async def get_galleries_by_user_admin(
    ...
    limit: int = Query(10, ge=1, le=100),
    offset: int = Query(0, ge=0),
) -> list[models.Gallery.GalleryModel]:

    with Session(c.db_engine) as session:
        galleries = session.exec(select(models.Gallery).where(
            models.Gallery.GalleryModel.user_id == user_id).offset(offset).limit(limit)).all()
        return galleries
...
async def sync_gallery(
    ...
    with Session(c.db_engine) as session:
        gallery = await models.Gallery.GalleryModel.api_get(session=session, c=c, authorized_user_id=authorization.user.id, id=gallery_id, admin=False)
...
class GetProfilePageResponse(GetAuthReturn):
    user: models.User.Private | None = None


@ pages_router.get('/profile/', tags=[models.User.UserModel._ROUTER_TAG])
...
class GetSettingsApiKeysPageResponse(GetAuthReturn):
    api_key_count: int
    api_keys: list[models.ApiKey.Private]


@ pages_router.get('/settings/api-keys/', tags=[models.ApiKey.ApiKeyModel._ROUTER_TAG])
...
class GetSettingsUserAccessTokensPageResponse(GetAuthReturn):
    ...
    user_access_tokens: list[models.UserAccessToken.UserAccessTokenModel]


@ pages_router.get('/settings/user-access-tokens/', tags=[models.UserAccessToken.UserAccessTokenModel._ROUTER_TAG])
...
class GetGalleryPageResponse(GetAuthReturn):
    gallery: models.Gallery.Public
    parents: list[models.Gallery.Public]
    children: list[models.Gallery.Public]


@ pages_router.get('/galleries/{gallery_id}/', responses={status.HTTP_404_NOT_FOUND: {"description": models.Gallery.GalleryModel.not_found_message(), 'model': NotFoundResponse}}, tags=[models.Gallery.GalleryModel._ROUTER_TAG])
    ...
    if root:
        with Session(c.db_engine) as session:
            if not authorization.isAuthorized:
                raise HTTPException(status.HTTP_404_NOT_FOUND,
                                    detail=models.Gallery.GalleryModel.not_found_message())

            gallery = await models.Gallery.GalleryModel.get_root_gallery(session, authorization.user.id)
            gallery_id = gallery._id

    with Session(c.db_engine) as session:
        gallery = await models.Gallery.GalleryModel.api_get(
        ...
        return GetGalleryPageResponse(
            **get_auth(authorization).model_dump(),
            gallery=models.Gallery.Public.model_validate(gallery),
            parents=[models.Gallery.Public.model_validate(
                parent) for parent in await gallery.get_parents(session)],
            children=[models.Gallery.Public.model_validate(
