...
async def get_authorization(
    ...
    if issubclass(AuthCredentialModel, models.Table):
        ...
        with Session(c.db_engine) as session:
            ...
            user: models.UserModel = auth_credential_from_db.user
            if user == None:
...
async def authenticate_user_with_username_and_password(form_data: typing.Annotated[OAuth2PasswordRequestForm, Depends()]) -> models.UserModel:
    with Session(c.db_engine) as session:
        user = await models.UserModel.authenticate(
...
async def post_token(
    user: typing.Annotated[models.UserModel, Depends(authenticate_user_with_username_and_password)],
    response: Response,
    stay_signed_in: bool = Form(c.authentication['stay_signed_in_default'])
) -> TokenResponse:
    with Session(c.db_engine) as session:
        user_access_token = await models.UserAccessTokenModel.api_post(
...
async def post_login_otp(
    ...
    with Session(c.db_engine) as session:
        ...
        if 'email' in model.model_fields_set:
            user = session.exec(select(models.UserModel).where(
                models.UserModel.email == model.email)).one_or_none()
        elif 'phone_number' in model.model_fields_set:
            user = session.exec(select(models.UserModel).where(
                models.UserModel.phone_number == model.phone_number)).one_or_none()
        ...
        hashed_code = models.OTPModel.hash_code(model.code)
        otp = session.exec(select(models.OTPModel).where(models.OTPModel.user_id == user.id).where(
            models.OTPModel.hashed_code == hashed_code)).one_or_none()
...
async def post_login_google(request_token: LoginWithGoogleRequest, response: Response) -> LoginWithGoogleResponse:
    ...
    with Session(c.db_engine) as session:
        ...
        user = session.exec(select(models.UserModel).where(
            models.UserModel.email == email)).one_or_none()
        if not user:
            user = await models.UserModel.api_post(session=session, c=c, authorized_user_id=None, create_model=models.User.AdminCreate(email=email, user_role_id=c.user_role_name_mapping['user']))
...
async def post_signup(response: Response, token: str = Query(None)) -> PostSignUpResponse:
    ...
    with Session(c.db_engine) as session:
        sign_up: models.SignUpModel = authorization.auth_credential

        user_create_admin = models.User.AdminCreate(
            email=sign_up.email, user_role_id=c.user_role_name_mapping['user'])

        user = await models.UserModel.api_post(session=session, c=c, authorized_user_id=None,
...
@ auth_router.post('/request-signup/', responses={status.HTTP_409_CONFLICT: {"description": models.UserModel.already_exists_message(), 'model': DetailOnlyResponse}})
async def post_request_sign_up(
    ...
    with Session(c.db_engine) as session:
        ...
        # send them a sign up link
            ...
            sign_up = models.SignUpModel.create(
...
async def post_send_otp(model: RequestOTPRequest, background_tasks: BackgroundTasks) -> DetailOnlyResponse:
    ...
    if 'email' in model.model_fields_set or 'phone_number' in model.model_fields_set:
        code = models.OTPModel.generate_code()
...
async def logout(response: Response, authorization: typing.Annotated[GetAuthorizationReturn, Depends(
    ...
    if authorization.auth_credential:
        if authorization.auth_credential.auth_type == 'access_token':
            with Session(c.db_engine) as session:
                await models.UserAccessTokenModel.api_delete(session=session, c=c, authorized_user_id=authorization._user_id, id=authorization.auth_credential.id, admin=False)
...
user_router = APIRouter(
    prefix='/users', tags=[models.UserModel._ROUTER_TAG])
user_admin_router = APIRouter(
    prefix='/admin/users', tags=[models.UserModel._ROUTER_TAG, 'Admin'])


@ user_router.get('/me/', responses=models.UserModel.get_responses())
...
@ user_router.get('/{user_id}/', responses=models.UserModel.get_responses())
async def get_user_by_id(
    ...
    with Session(c.db_engine) as session:
        return models.User.Public.model_validate(await models.UserModel.api_get(session=session, c=c, authorized_user_id=authorization._user_id, id=user_id, admin=False))


@ user_admin_router.get('/{user_id}/', responses=models.UserModel.get_responses())
async def get_user_by_id_admin(
    ...
    with Session(c.db_engine) as session:
        a = await models.UserModel.api_get(session=session, c=c, authorized_user_id=authorization._user_id, id=user_id, admin=True)


@ user_admin_router.post('/', responses=models.UserModel.post_responses())
async def post_user_admin(
    ...
    with Session(c.db_engine) as session:
        return await models.UserModel.api_post(session=session, c=c, authorized_user_id=authorization._user_id, admin=True, create_model=user_create_admin)


@ user_router.patch('/me/', responses=models.UserModel.patch_responses())
async def patch_self(
    ...
    with Session(c.db_engine) as session:
        return models.User.Private.model_validate(await models.UserModel.api_patch(session=session, c=c, authorized_user_id=authorization._user_id, id=authorization._user_id, admin=False, update_model=models.UserUpdateAdmin(**user_update.model_dump(exclude_unset=True))))


@ user_admin_router.patch('/{user_id}/', responses=models.UserModel.patch_responses())
async def patch_user_admin(
    ...
    with Session(c.db_engine) as session:
        return await models.UserModel.api_patch(session=session, c=c, authorized_user_id=authorization._user_id, id=user_id, admin=True, update_model=user_update_admin)


@ user_router.delete('/me/', responses=models.UserModel.delete_responses(), status_code=status.HTTP_204_NO_CONTENT)
async def delete_self(
    authorization: typing.Annotated[GetAuthorizationReturn, Depends(
        make_get_auth_dependency())]
):
    with Session(c.db_engine) as session:
        return await models.UserModel.api_delete(session=session, c=c, authorized_user_id=authorization._user_id, id=authorization._user_id, admin=False)


@ user_admin_router.delete('/{user_id}/', responses=models.UserModel.delete_responses(), status_code=status.HTTP_204_NO_CONTENT)
async def delete_user_admin(
    ...
    with Session(c.db_engine) as session:
        return await models.UserModel.api_delete(session=session, c=c, authorized_user_id=authorization._user_id, id=user_id, admin=True)


@ user_router.get('/', responses=models.UserModel.post_responses())
async def get_users(
    ...
    with Session(c.db_engine) as session:
        query = select(models.UserModel).where(
            models.UserModel.username != None)
...
async def get_user_username_available(username: models.UserTypes.username):
    with Session(c.db_engine) as session:
        return await models.UserModel.api_get_is_username_available(session, username)
...
user_access_token_router = APIRouter(
    prefix='/user-access-tokens', tags=[models.UserAccessTokenModel._ROUTER_TAG])
user_access_token_admin_router = APIRouter(
    prefix='/admin/user-access-tokens', tags=[models.UserAccessTokenModel._ROUTER_TAG, 'Admin'])


@ user_access_token_router.get('/{user_access_token_id}/', responses=models.UserAccessTokenModel.get_responses())
async def get_user_access_token_by_id(
    user_access_token_id: models.UserAccessTokenTypes.id,
    authorization: typing.Annotated[GetAuthorizationReturn, Depends(
        make_get_auth_dependency())]
) -> models.UserAccessTokenModel:
    with Session(c.db_engine) as session:
        return await models.UserAccessTokenModel.api_get(session=session, c=c, authorized_user_id=authorization._user_id, id=user_access_token_id, admin=False)


@ user_access_token_admin_router.get('/{user_access_token_id}/', responses=models.UserAccessTokenModel.get_responses())
async def get_user_access_token_by_id_admin(
    ...
    with Session(c.db_engine) as session:
        return await models.UserAccessTokenModel.api_get(session=session, c=c, authorized_user_id=authorization._user_id, id=user_access_token_id, admin=True)


@ user_access_token_admin_router.post('/', responses=models.UserAccessTokenModel.post_responses())
async def post_user_access_token_admin(
    ...
    with Session(c.db_engine) as session:
        return await models.UserAccessTokenModel.api_post(session=session, c=c, authorized_user_id=authorization._user_id, admin=True, create_model=user_access_token_create_admin)


@ user_access_token_router.delete('/{user_access_token_id}/', responses=models.UserAccessTokenModel.delete_responses(), status_code=status.HTTP_204_NO_CONTENT)
async def delete_user_access_token(
    ...
    with Session(c.db_engine) as session:
        return await models.UserAccessTokenModel.api_delete(session=session, c=c, authorized_user_id=authorization._user_id, id=user_access_token_id, admin=False)


@ user_access_token_admin_router.delete('/{user_access_token_id}/', responses=models.UserAccessTokenModel.delete_responses(), status_code=status.HTTP_204_NO_CONTENT)
async def delete_user_access_token_admin(
    ...
    with Session(c.db_engine) as session:
        return await models.UserAccessTokenModel.api_delete(session=session, c=c, authorized_user_id=authorization._user_id, id=user_access_token_id, admin=True)
...
@ user_access_token_router.get('/', tags=[models.UserModel._ROUTER_TAG], responses={status.HTTP_404_NOT_FOUND: {"description": models.UserModel.not_found_message(), 'model': NotFoundResponse}, WrongUserPermissionUserAccessToken.status_code: {'description': WrongUserPermissionUserAccessToken.detail, 'model': DetailOnlyResponse}})
async def get_user_access_tokens(
    authorization: typing.Annotated[GetAuthorizationReturn, Depends(
        make_get_auth_dependency())],
    pagination: models.Pagination = Depends(user_access_token_pagination)

) -> list[models.UserAccessTokenModel]:
    with Session(c.db_engine) as session:
        query = select(models.UserAccessTokenModel).where(
            models.UserAccessTokenModel.user_id == authorization._user_id)
        query = models.build_pagination(query, pagination)
        user_access_tokens = session.exec(query).all()
        return user_access_tokens


@ user_access_token_admin_router.get('/users/{user_id}/', tags=[models.UserModel._ROUTER_TAG], responses={status.HTTP_404_NOT_FOUND: {"description": models.UserModel.not_found_message(), 'model': NotFoundResponse}})
async def get_user_access_tokens_admin(
    ...
    with Session(c.db_engine) as session:
        ...
        query = select(models.UserAccessTokenModel).where(
            models.UserAccessTokenModel.user_id == user_id)
        query = models.build_pagination(query, pagination)
        user_access_tokens = session.exec(query).all()
        return user_access_tokens
...
async def get_user_access_tokens_count(
    ...
    with Session(c.db_engine) as session:
        query = select(func.count()).select_from(models.UserAccessTokenModel).where(
            models.UserAccessTokenModel.user_id == authorization._user_id)
...
api_key_router = APIRouter(
    prefix='/api-keys', tags=[models.ApiKeyModel._ROUTER_TAG])
api_key_admin_router = APIRouter(
    prefix='/admin/api-keys', tags=[models.ApiKeyModel._ROUTER_TAG, 'Admin'])


@ api_key_router.get('/{api_key_id}/', responses=models.ApiKeyModel.get_responses())
async def get_api_key_by_id(
    ...
    with Session(c.db_engine) as session:
        return models.ApiKey.Private.model_validate(await models.ApiKeyModel.api_get(session=session, c=c, authorized_user_id=authorization.user.id, id=api_key_id, admin=False))


@ api_key_admin_router.get('/{api_key_id}/', responses=models.ApiKeyModel.get_responses())
async def get_api_key_by_id_admin(
    ...
    with Session(c.db_engine) as session:
        return await models.ApiKeyModel.api_get(session=session, c=c, authorized_user_id=authorization.user.id, id=api_key_id, admin=True)


@ api_key_router.post('/', responses=models.ApiKeyModel.post_responses())
async def post_api_key_to_user(
    ...
    with Session(c.db_engine) as session:
        return models.ApiKey.Private.from_api_key(await models.ApiKeyModel.api_post(session=session, c=c, authorized_user_id=authorization._user_id, admin=False, create_model=models.ApiKey.AdminCreate(
            **api_key_create.model_dump(exclude_unset=True), user_id=authorization._user_id)))


@ api_key_admin_router.post('/', responses=models.ApiKeyModel.post_responses())
async def post_api_key_to_user_admin(
    ...
    with Session(c.db_engine) as session:
        return await models.ApiKeyModel.api_post(session=session, c=c, authorized_user_id=authorization.user.id, admin=True, create_model=models.ApiKey.AdminCreate(**api_key_create_admin.model_dump(exclude_unset=True)))


@ api_key_router.patch('/{api_key_id}/', responses=models.ApiKeyModel.patch_responses())
async def patch_api_key(
    ...
    with Session(c.db_engine) as session:
        return models.ApiKey.Private.from_api_key(await models.ApiKeyModel.api_patch(session=session, c=c, authorized_user_id=authorization.user.id, id=api_key_id, admin=False, update_model=models.ApiKey.AdminUpdate(**api_key_update.model_dump(exclude_unset=True))))


@ api_key_admin_router.patch('/{api_key_id}/', responses=models.ApiKeyModel.patch_responses())
async def patch_api_key_admin(
    ...
    with Session(c.db_engine) as session:
        return await models.ApiKeyModel.api_patch(session=session, c=c, authorized_user_id=authorization.user.id, id=api_key_id, admin=True, update_model=models.ApiKey.AdminUpdate(**api_key_update_admin.model_dump(exclude_unset=True)))


@ api_key_router.delete('/{api_key_id}/', responses=models.ApiKeyModel.delete_responses(), status_code=status.HTTP_204_NO_CONTENT)
async def delete_api_key(
    ...
    with Session(c.db_engine) as session:
        return await models.ApiKeyModel.api_delete(session=session, c=c, authorized_user_id=authorization.user.id, id=api_key_id, admin=False)


@ api_key_admin_router.delete('/{api_key_id}/', responses=models.ApiKeyModel.delete_responses(), status_code=status.HTTP_204_NO_CONTENT)
async def delete_api_key_admin(
    ...
    with Session(c.db_engine) as session:
        return await models.ApiKeyModel.api_delete(session=session, c=c, authorized_user_id=authorization.user.id, id=api_key_id, admin=True)
...
@ api_key_router.get('/{api_key_id}/generate-jwt/', responses={status.HTTP_404_NOT_FOUND: {"description": models.ApiKeyModel.not_found_message(), 'model': NotFoundResponse}})
async def get_api_key_jwt(
    ...
    with Session(c.db_engine) as session:
        api_key = await models.ApiKeyModel.api_get(session=session, c=c, authorized_user_id=authorization.user.id, id=api_key_id, admin=False)
...
def get_api_keys_query_params(
    ...
    order_bys: typing.Annotated[list[models.OrderBy[models.ApiKeyTypes.order_by]], Depends(
        models.ApiKeyModel.get_order_by_depends())]
):
...
async def get_user_api_keys(
    ...
    with Session(c.db_engine) as session:
        ...
        query = select(models.ApiKeyModel).options(selectinload(models.ApiKeyModel.api_key_scopes)).where(
            models.ApiKeyModel.user_id == authorization._user_id)
        query = models.build_pagination(
            query, get_api_keys_query_params.pagination)
        query = models.ApiKeyModel._build_order_by(
...
async def get_user_api_keys_count(
    ...
    with Session(c.db_engine) as session:
        query = select(func.count()).select_from(models.ApiKeyModel).where(
            models.ApiKeyModel.user_id == authorization._user_id)
        return session.exec(query).one()


@ api_key_admin_router.get('/users/{user_id}/', tags=[models.UserModel._ROUTER_TAG])
async def get_user_api_keys_admin(
    ...
    get_api_keys_query_params: GetApiKeysQueryParamsReturn = Depends(
        get_api_keys_query_params)
) -> list[models.ApiKeyModel]:
    with Session(c.db_engine) as session:

        query = select(models.ApiKeyModel).where(
            models.ApiKeyModel.user_id == user_id)
        query = models.build_pagination(
            query, get_api_keys_query_params.pagination)
        query = models.ApiKeyModel._build_order_by(
            query, get_api_keys_query_params.order_bys)
...
@ api_key_router.get('/details/available/', responses={status.HTTP_409_CONFLICT: {"description": models.ApiKeyModel.already_exists_message(), 'model': DetailOnlyResponse}})
async def get_api_key_available(
    ...
    with Session(c.db_engine) as session:
        await models.ApiKeyModel.api_get_is_available(session, models.ApiKey.AdminAvailable(
            **api_key_available.model_dump(exclude_unset=True), user_id=authorization.user.id
        ))


@ api_key_admin_router.get('/details/available/', responses={status.HTTP_409_CONFLICT: {"description": models.ApiKeyModel.already_exists_message(), 'model': DetailOnlyResponse}})
async def get_api_key_available_admin(
    ...
    with Session(c.db_engine) as session:
        await models.ApiKeyModel.api_get_is_available(session, api_key_available_admin)
...
api_key_scope_router = APIRouter(
    prefix='/api-key-scopes', tags=[models.ApiKeyScopeModel._ROUTER_TAG])
api_key_scope_admin_router = APIRouter(
    prefix='/admin/api-key-scopes', tags=[models.ApiKeyScopeModel._ROUTER_TAG, 'Admin'])


@ api_key_scope_router.post('/api-keys/{api_key_id}/scopes/{scope_id}/',
                            responses={status.HTTP_404_NOT_FOUND: {
                                "description": models.ApiKeyModel.not_found_message(), 'model': NotFoundResponse}},
                            )
async def add_scope_to_api_key(
    ...
    with Session(c.db_engine) as session:
        return await models.ApiKeyScopeModel.api_post(session=session, c=c, authorized_user_id=authorization.user.id, create_model=models.ApiKeyScope.AdminCreate(api_key_id=api_key_id, scope_id=scope_id), admin=False)
...
async def add_scope_to_api_key_admin(
    ...
    with Session(c.db_engine) as session:
        return await models.ApiKeyScopeModel.api_post(session=session, c=c, authorized_user_id=authorization.user.id, create_model=models.ApiKeyScope.AdminCreate(api_key_id=api_key_id, scope_id=scope_id), admin=True)
...
async def remove_scope_from_api_key(
    ...
    with Session(c.db_engine) as session:
        return await models.ApiKeyScopeModel.api_delete(session=session, c=c, authorized_user_id=authorization.user.id, id=models.ApiKeyScope.IdBase(api_key_id=api_key_id, scope_id=scope_id)._id, admin=False)
...
async def remove_scope_from_api_key_admin(
    ...
    with Session(c.db_engine) as session:
        return await models.ApiKeyScopeModel.api_delete(session=session, c=c, authorized_user_id=authorization.user.id, id=models.ApiKeyScope.IdBase(api_key_id=api_key_id, scope_id=scope_id)._id, admin=True)
...
gallery_router = APIRouter(
    prefix='/galleries', tags=[models.GalleryModel._ROUTER_TAG])
gallery_admin_router = APIRouter(prefix='/admin/galleries', tags=[
    models.GalleryModel._ROUTER_TAG, 'Admin'])


@ gallery_router.get('/{gallery_id}/', responses=models.GalleryModel.get_responses())
async def get_gallery_by_id(
    ...
    with Session(c.db_engine) as session:
        return models.Gallery.Public.model_validate(await models.GalleryModel.api_get(session=session, c=c, authorized_user_id=authorization._user_id, id=gallery_id, admin=False))


@ gallery_admin_router.get('/{gallery_id}/', responses=models.GalleryModel.get_responses())
async def get_gallery_by_id_admin(
    ...
    with Session(c.db_engine) as session:
        return await models.GalleryModel.api_get(session=session, c=c, authorized_user_id=authorization._user_id, id=gallery_id, admin=True)


@ gallery_router.post('/', responses=models.GalleryModel.post_responses())
async def post_gallery(
    ...
    with Session(c.db_engine) as session:
        return models.Gallery.Private.model_validate(await models.GalleryModel.api_post(session=session, c=c, authorized_user_id=authorization._user_id, admin=False, create_model=models.Gallery.AdminCreate(**gallery_create.model_dump(exclude_unset=True), user_id=authorization._user_id)))


@ gallery_admin_router.post('/', responses=models.GalleryModel.post_responses())
async def post_gallery_admin(
    ...
    with Session(c.db_engine) as session:
        return await models.GalleryModel.api_post(session=session, c=c, authorized_user_id=authorization._user_id, admin=True, create_model=models.Gallery.AdminCreate(**gallery_create_admin.model_dump(exclude_unset=True)))


@ gallery_router.patch('/{gallery_id}/', responses=models.GalleryModel.patch_responses())
async def patch_gallery(
    ...
    with Session(c.db_engine) as session:
        return models.Gallery.Private.model_validate(await models.GalleryModel.api_patch(session=session, c=c, authorized_user_id=authorization._user_id, id=gallery_id, admin=False, update_model=models.Gallery.AdminUpdate(**gallery_update.model_dump(exclude_unset=True))))


@ gallery_admin_router.patch('/{gallery_id}/', responses=models.GalleryModel.patch_responses())
async def patch_gallery_admin(
    ...
    with Session(c.db_engine) as session:
        return await models.GalleryModel.api_patch(session=session, c=c, authorized_user_id=authorization._user_id, id=gallery_id, admin=True, update_model=models.Gallery.AdminUpdate(**gallery_update_admin.model_dump(exclude_unset=True)))


@ gallery_router.delete('/{gallery_id}/', responses=models.GalleryModel.delete_responses(), status_code=status.HTTP_204_NO_CONTENT)
async def delete_gallery(
    ...
    with Session(c.db_engine) as session:
        return await models.GalleryModel.api_delete(session=session, c=c, authorized_user_id=authorization._user_id, id=gallery_id, admin=False)


@ gallery_admin_router.delete('/{gallery_id}/', responses=models.GalleryModel.delete_responses(), status_code=status.HTTP_204_NO_CONTENT)
async def delete_gallery_admin(
    ...
    with Session(c.db_engine) as session:
        return await models.GalleryModel.api_delete(session=session, c=c, authorized_user_id=authorization._user_id, id=gallery_id, admin=True)
...
async def get_gallery_available(
    ...
    with Session(c.db_engine) as session:
        await models.GalleryModel.api_get_is_available(session, models.Gallery.AdminAvailable(
...
async def get_gallery_available_admin(
    ...
    with Session(c.db_engine) as session:
        await models.GalleryModel.api_get_is_available(session, gallery_available_admin)
...
async def get_galleries(
    ...
    with Session(c.db_engine) as session:
        galleries = session.exec(select(models.GalleryModel).where(
            models.GalleryModel.user_id == authorization._user_id).offset(offset).limit(limit)).all()
...
@ gallery_admin_router.get('/users/{user_id}', tags=[models.UserModel._ROUTER_TAG])
async def get_galleries_by_user_admin(
    ...
    limit: int = Query(10, ge=1, le=100),
    offset: int = Query(0, ge=0),
) -> list[models.GalleryModel]:

    with Session(c.db_engine) as session:
        galleries = session.exec(select(models.GalleryModel).where(
            models.GalleryModel.user_id == user_id).offset(offset).limit(limit)).all()
        return galleries
...
async def sync_gallery(
    ...
    with Session(c.db_engine) as session:
        gallery = await models.GalleryModel.api_get(session=session, c=c, authorized_user_id=authorization.user.id, id=gallery_id, admin=False)
...
@ pages_router.get('/profile/', tags=[models.UserModel._ROUTER_TAG])
...
@ pages_router.get('/settings/api-keys/', tags=[models.ApiKeyModel._ROUTER_TAG])
...
class GetSettingsUserAccessTokensPageResponse(GetAuthReturn):
    ...
    user_access_tokens: list[models.UserAccessTokenModel]


@ pages_router.get('/settings/user-access-tokens/', tags=[models.UserAccessTokenModel._ROUTER_TAG])
...
@ pages_router.get('/galleries/{gallery_id}/', responses={status.HTTP_404_NOT_FOUND: {"description": models.GalleryModel.not_found_message(), 'model': NotFoundResponse}}, tags=[models.GalleryModel._ROUTER_TAG])
    ...
    if root:
        with Session(c.db_engine) as session:
            if not authorization.isAuthorized:
                raise HTTPException(status.HTTP_404_NOT_FOUND,
                                    detail=models.GalleryModel.not_found_message())

            gallery = await models.GalleryModel.get_root_gallery(session, authorization.user.id)
            gallery_id = gallery._id

    with Session(c.db_engine) as session:
        gallery = await models.GalleryModel.api_get(
