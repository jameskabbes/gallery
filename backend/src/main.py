...
) -> typing.Callable[[str], models.GetAuthorizationReturn]:

    async def dependecy(response: Response, auth_token: typing.Annotated[str | None, Depends(oauth2_scheme)]) -> models.GetAuthorizationReturn:
        async def inner() -> models.GetAuthorizationReturn:
            # attempt to decode the jwt

            if auth_token == None:
                return models.GetAuthorizationReturn(exception='improper_format')
            ...
            if not models.AuthCredential.validate_jwt_claims(payload):
                return models.GetAuthorizationReturn(exception='missing_required_claims')

            # make sure the type is permitted
            if payload['type'] not in permitted_auth_credential_types:
                return models.GetAuthorizationReturn(exception='invalid_authorization_type')
            ...
            with Session(c.db_engine) as session:
                ...
                if not auth_credential:
                    return models.GetAuthorizationReturn(exception='authorization_expired')
                ...
                if user == None:
                    return models.GetAuthorizationReturn(exception='user_not_found')
                ...
                if not required_scope_ids.issubset(scope_ids):
                    return models.GetAuthorizationReturn(exception='not_permitted')

                return models.GetAuthorizationReturn(
                    isAuthorized=True,
                    expiry=dt_exp,
...
def get_auth(get_authorization_return: models.GetAuthorizationReturn) -> GetAuthReturn:
...
@ app.get('/auth/')
async def auth_root(authorization: typing.Annotated[models.GetAuthorizationReturn, Depends(get_authorization(raise_exceptions=False))]) -> GetAuthReturn:
...
async def post_token(
    ...
    with Session(c.db_engine) as session:
        auth_credential = await models.UserAccessTokenCreateAdmin(
            user_id=user.id, lifespan=c.authentication['default_expiry_timedelta']).post(session)
...
async def sign_up(
    ...
    with Session(c.db_engine) as session:
        user = await user_create_admin.post(session, c)
        user_access_token = await models.UserAccessTokenCreateAdmin(
            user_id=user.id, type='access_token', lifespan=c.authentication['default_expiry_timedelta']).post(session)
        set_access_token_cookie(jwt_encode(
            user_access_token.export_to_jwt()), response, stay_signed_in)
...
async def verify_magic_link(model: VerifyMagicLinkRequest, authorization: typing.Annotated[models.GetAuthorizationReturn, Depends(get_authorization(permitted_auth_credential_types={'access_token'},
...
async def logout(response: Response, authorization: typing.Annotated[models.GetAuthorizationReturn, Depends(
...
async def get_user_by_id_admin(
    ...
    authorization: typing.Annotated[models.GetAuthorizationReturn, Depends(
        get_authorization(required_scopes={'admin'}))]
) -> models.User:
    with Session(c.db_engine) as session:
        return await models.User.api_get(session, c, authorization, user_id, admin=True)
...
async def post_user_admin(
    ...
    with Session(c.db_engine) as session:
        return await user_create_admin.post(session, c)
...
async def get_user(authorization: typing.Annotated[models.GetAuthorizationReturn, Depends(get_authorization())]) -> models.UserPrivate:
    with Session(c.db_engine) as session:
        return models.UserPrivate.model_validate(await models.User.api_get(session, authorization.user.id))
...
async def get_user_by_id(
    ...
    with Session(c.db_engine) as session:
        return models.UserPublic.model_validate(await models.User.api_get(session, user_id))
...
async def get_user_access_tokens(
        authorization: typing.Annotated[models.GetAuthorizationReturn, Depends(get_authorization())]) -> list[models.UserAccessToken]:
    with Session(c.db_engine) as session:
...
async def delete_user_access_token(
        user_access_token_id: models.UserAccessTokenTypes.id,
        authorization: typing.Annotated[models.GetAuthorizationReturn, Depends(get_authorization())]):
...
async def get_api_keys(
        authorization: typing.Annotated[models.GetAuthorizationReturn, Depends(get_authorization())]) -> list[models.ApiKey]:
...
async def post_api_key(
        ...
        authorization: typing.Annotated[models.GetAuthorizationReturn, Depends(get_authorization())]) -> models.ApiKey:
...
async def post_gallery_admin(
    ...
    with Session(c.db_engine) as session:
        return await gallery_create_admin.post(session, c)
...
async def get_gallery(
    ...
    with Session(c.db_engine) as session:
        gallery = await models.Gallery.api_get(session, c, authorization, gallery_id)
...
async def post_gallery(
    ...
    with Session(c.db_engine) as session:
        gallery = await gallery_create_admin.post(session, c)
...
async def patch_gallery(gallery_id: models.GalleryTypes.id,
    ...
    with Session(c.db_engine) as session:
        return await models.GalleryUpdateAdmin(**gallery_update.model_dump(exclude_unset=True)).api_patch(session)
...
async def get_pages_profile(authorization: typing.Annotated[models.GetAuthorizationReturn, Depends(
...
@ app.get('/home/page/')
async def get_home_page(authorization: typing.Annotated[models.GetAuthorizationReturn, Depends(get_authorization(raise_exceptions=False))]) -> GetHomePageResponse:
...
async def get_settings_page(authorization: typing.Annotated[models.GetAuthorizationReturn, Depends(get_authorization(raise_exceptions=False))]) -> GetSettingsPageResponse:
...
async def get_settings_page(
        authorization: typing.Annotated[models.GetAuthorizationReturn, Depends(get_authorization())]) -> GetSettingsApiKeysPageResponse:
...
@ app.get('/styles/page/')
async def get_styles_page(authorization: typing.Annotated[models.GetAuthorizationReturn, Depends(get_authorization(raise_exceptions=False))]) -> GetStylesPageResponse:
...
async def get_gallery_page(
    ...
    with Session(c.db_engine) as session:
        g = await models.Gallery.get(session, gallery_id)
        parents = await g.get_parents(session)
        children = g.children
