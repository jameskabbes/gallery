...
class File(document_object.DocumentObject[None, None]):
    ...
    def find_to_add_and_delete(cls, collection: pymongo_collection.Collection, dir: pathlib.Path, event_id: types.EventId) -> tuple[set[tuple], set[types.MediaId]]:
        ...
        local_id_keys_by_media_type: dict[media_types.MEDIA_CORE_TYPES, set[tuple]] = {
        }

        for media_core_type in media_types.MEDIA_CORE_TYPES:
        ...
        for file in dir.iterdir():
            if file.is_file():
                ...
                media_core_type = media_types.get_media_core_type_from_file_ending(
                    file_ending)
    ...
    @classmethod
    def make_from_id_keys(cls, id_keys: tuple) -> media_types.FileClassTypes:

        media_core_type = id_keys[cls.MEDIA_TYPE_ID_KEY_INDEX]
        file_class = media_types.FILE_CLASS_MAPPING[media_core_type]
        return file_class.make_from_id_keys(id_keys)
