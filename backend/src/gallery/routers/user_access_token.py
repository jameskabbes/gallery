...
PREFIX = '/user_access_tokens'
TAG = 'User Access Token'
...
def user_access_token_pagination(
    pagination: Annotated[tables.Pagination, Depends(
        get_pagination(default_limit=50, max_limit=500))]
):
    return pagination


class UserAccessTokenRouter(base.Router[UserAccessTokenTable]):
    ...
    def _set_routes(self):
        ...
        async def get_user_access_token_by_id(
            ...
            async with self.client.AsyncSession() as session:
                return await UserAccessTokenService.read({
                    'session': session,
                    'c': self.client,
                    'admin': False,
