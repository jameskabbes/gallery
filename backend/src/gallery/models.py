...
class User(ModelManager):
    ...
    class UserModel(Table['UserModel', UserTypes.id, AdminCreate, AdminUpdate], UserBase, IdBase, table=True):
        __tablename__ = 'user'
        ...
        api_keys: list['ApiKey.ApiKeyModel'] = Relationship(
            back_populates='user', cascade_delete=True)
        user_access_tokens: list['UserAccessToken.UserAccessTokenModel'] = Relationship(
            back_populates='user', cascade_delete=True)
        galleries: list['Gallery.GalleryModel'] = Relationship(
            back_populates='user', cascade_delete=True)
        gallery_permissions: list['GalleryPermission.GalleryPermissionModel'] = Relationship(
            back_populates='user', cascade_delete=True)
        otps: list['OTP.OTPModel'] = Relationship(
            back_populates='user', cascade_delete=True)
        ...
        async def is_username_available(cls, session: Session, username: UserTypes.username) -> bool:
            ...
            if session.exec(select(cls).where(User.UserModel.username == username)).one_or_none():
                return False
            return True
        ...
        async def is_email_available(cls, session: Session, email: UserTypes.email) -> bool:
            if session.exec(select(cls).where(User.UserModel.email == email)).one_or_none():
                return False
            return True
        ...
        async def _check_validation_post(cls, **kwargs):
            ...
            if 'username' in model.model_fields_set:
                if model.username != None:
                    await User.UserModel.api_get_is_username_available(
                        kwargs['session'], model.username)
            if 'email' in model.model_fields_set:
                if model.email != None:
                    await User.UserModel.api_get_is_email_available(kwargs['session'], model.email)
        ...
        async def create(cls, **kwargs) -> typing.Self:
            ...
            root_gallery = await Gallery.GalleryModel.api_post(session=kwargs['session'], c=kwargs['c'], authorized_user_id=new_user._id, admin=kwargs['admin'], create_model=Gallery.AdminCreate(
        ...
        async def update(self, **kwargs):
            ...
            if 'username' in kwargs['update_model'].model_fields_set:
                root_gallery = await Gallery.GalleryModel.get_root_gallery(kwargs['session'], self._id)
        ...
        async def delete(self, **kwargs):
            await (await Gallery.GalleryModel.get_root_gallery(kwargs['session'], self._id)).delete(session=kwargs['session'], c=kwargs['c'],
...
class AuthCredential:
    ...
    class Model(BaseModel):
        ...
        _JWT_CLAIMS_MAPPING_ADDITIONAL: typing.ClassVar[dict[str, str]] = {}

        @classmethod
        def _JWT_CLAIMS_MAPPING(cls) -> dict[str, str]:
            return {**cls._JWT_CLAIMS_MAPPING_BASE, **cls._JWT_CLAIMS_MAPPING_ADDITIONAL}
...
class UserAccessToken(ModelManager):
    ...
    class UserAccessTokenModel(
        ...
        user_id: UserTypes.id = Field(
            index=True, foreign_key=User.UserModel.__tablename__ + '.' + User.UserModel._ID_COLS[0], const=True, ondelete='CASCADE')

        user: 'User.UserModel' = Relationship(
            back_populates='user_access_tokens')

        _JWT_CLAIMS_MAPPING_ADDITIONAL = {
            'sub': 'id'}
...
class ApiKey(ModelManager):
    ...
    class Private(Export):
        ...
        @classmethod
        def from_api_key(cls, api_key: 'ApiKey.ApiKeyModel') -> typing.Self:
    ...
    class ApiKeyModel(
        ...
        user: 'User.UserModel' = Relationship(back_populates='api_keys')
        api_key_scopes: list['ApiKeyScope.ApiKeyScopeModel'] = Relationship(
            back_populates='api_key', cascade_delete=True)

        _JWT_CLAIMS_MAPPING_ADDITIONAL = {'sub': 'id'}
        _ROUTER_TAG = 'Api Key'
...
class SignUp:
    ...
    class SignUpModel(
        ...
        _JWT_CLAIMS_MAPPING_ADDITIONAL = {'sub': 'email'}

        @classmethod
        def create(cls, create_model: 'SignUp.AdminCreate') -> 'SignUp.SignUpModel':
            return cls(
                issued=datetime_module.datetime.now(
                    datetime_module.timezone.utc),
...
AuthCredentialModel = UserAccessToken.UserAccessTokenModel | ApiKey.ApiKeyModel | OTP.OTPModel | SignUp.SignUpModel
...
AUTH_CREDENTIAL_MODELS: set[AuthCredentialModel] = {
    UserAccessToken.UserAccessTokenModel, ApiKey.ApiKeyModel, OTP.OTPModel, SignUp.SignUpModel}
...
AUTH_CREDENTIAL_TYPE_TO_MODEL: dict[AuthCredentialTypes.type, AuthCredentialModel] = {
    'access_token': UserAccessToken.UserAccessTokenModel,
    'api_key': ApiKey.ApiKeyModel,
    'otp': OTP.OTPModel,
    'sign_up': SignUp.SignUpModel
}
...
class ApiKeyScope(ModelManager):
    ...
    class IdBase(IdObject[ApiKeyScopeTypes.id]):
        ...
        api_key_id: ApiKeyTypes.id = Field(
            index=True, foreign_key=ApiKey.ApiKeyModel.__tablename__ + '.' + ApiKey.ApiKeyModel._ID_COLS[0], ondelete='CASCADE')
    ...
    class ApiKeyScopeModel(
        ...
        api_key: ApiKey.ApiKeyModel = Relationship(
        ...
        @ classmethod
        async def _check_authorization_post(cls, **kwargs):
            api_key = await ApiKey.ApiKeyModel._basic_api_get(kwargs['session'], kwargs['create_model'].api_key_id)
...
class Gallery:
    ...
    class GalleryModel(
        ...
        user_id: GalleryTypes.user_id = Field(
            index=True, foreign_key=User.UserModel.__tablename__ + '.' + User.UserModel._ID_COLS[0], ondelete='CASCADE')
        ...
        user: 'User.UserModel' = Relationship(back_populates='galleries')
        parent: typing.Optional['Gallery.GalleryModel'] = Relationship(
            back_populates='children', sa_relationship_kwargs={'remote_side': 'Gallery.id'})
        children: list['Gallery.GalleryModel'] = Relationship(
            back_populates='parent', cascade_delete=True)
        gallery_permissions: list['GalleryPermission.GalleryPermissionModel'] = Relationship(
            back_populates='gallery', cascade_delete=True)
        files: list['File.FileModel'] = Relationship(
            back_populates='gallery', cascade_delete=True)
        image_versions: list['ImageVersion.ImageVersionModel'] = Relationship(
            back_populates='gallery', cascade_delete=True)
        ...
        async def _check_authorization_existing(self, **kwargs):
            ...
            if not kwargs['admin']:
                if kwargs['authorized_user_id'] != self.user_id:
                    ...
                    gallery_permission = await GalleryPermission.GalleryPermissionModel.get_one_by_id(kwargs['session'], (kwargs['id'], kwargs['authorized_user_id']))
        ...
        async def sync_with_local(self, session: Session, c: client.Client, dir: pathlib.Path) -> None:
            ...
            for folder_name in to_remove:
                ...
                await Gallery.GalleryModel.api_delete(session=session, c=c, id=gallery.id, authorized_user_id=self.user_id, admin=False, rmtree=False)

            for folder_name in to_add:
                date, name = self.get_date_and_name_from_folder_name(
                    folder_name)
                new_gallery = await Gallery.GalleryModel.api_post(
            ...
            for file_name in to_remove:
                ...
                if file.suffix in ImageFileMetadata._SUFFIXES:
                    image_version = await ImageVersion.ImageVersionModel.get_one_by_id(session, file.image_file_metadata.version_id)
                    if len(image_version.image_file_metadatas) == 1:
                        await ImageVersion.ImageVersionModel.api_delete(session=session, c=c, id=image_version.id, authorized_user_id=self.user_id, admin=False)

                await File.FileModel.api_delete(session=session, c=c, id=file.id, authorized_user_id=self.user_id, admin=False, unlink=False)

            image_files: list[File.FileModel] = []
            ...
            for file_name in to_add:
                ...
                new_file = await File.FileModel.api_post(
            ...
            for original_images in [True, False]:
                for image_file in image_files:
                    ...
                    base_name, version, scale = ImageFileMetadata.ImageFileMetadataModel.parse_file_stem(
                        image_file.stem)
                    ...
                    if original_images == (version == None):
                        ...
                        if version is not None:
                            image_version_og = session.exec(select(ImageVersion.ImageVersionModel).where(ImageVersion.ImageVersionModel.gallery_id == self._id).where(
                                ImageVersion.ImageVersionModel.base_name == base_name).where(ImageVersion.ImageVersionModel.version == None)).one_or_none()
                        ...
                        image_version = session.exec(select(ImageVersion.ImageVersionModel).where(
                            ImageVersion.ImageVersionModel._build_conditions(image_version_kwargs))).one_or_none()

                        # this if the first file of this version
                        if not image_version:
                            image_version = await ImageVersion.ImageVersionModel.api_post(session=session, c=c, authorized_user_id=self.user_id, admin=False, create_model=ImageVersion.AdminCreate(**image_version_kwargs))

                        image_file_metadata = await ImageFileMetadata.ImageFileMetadataModel.api_post(session=session, c=c, authorized_user_id=self.user_id, admin=False, create_model=ImageFileMetadata.AdminCreate(file_id=image_file.id, version_id=image_version.id, scale=scale))
...
class GalleryPermission:
    ...
    class IdBase(IdObject[GalleryPermissionTypes.id]):
        ...
        gallery_id: GalleryPermissionTypes.gallery_id = Field(
            primary_key=True, index=True, foreign_key=Gallery.GalleryModel.__tablename__ + '.' + Gallery.GalleryModel._ID_COLS[0], ondelete='CASCADE')
        user_id: GalleryPermissionTypes.user_id = Field(
            primary_key=True, index=True, foreign_key=User.UserModel.__tablename__ + '.' + User.UserModel._ID_COLS[0], ondelete='CASCADE')
    ...
    class GalleryPermissionModel(
            Table['GalleryPermissionModel',
                  GalleryPermissionTypes.id, AdminCreate, AdminUpdate],
            IdBase,
            table=True):
        ...
        gallery: 'Gallery.GalleryModel' = Relationship(
        ...
        @ classmethod
        async def _check_validation_post(cls, **kwargs):
            if await GalleryPermission.GalleryPermissionModel.get_one_by_id(kwargs['session'], kwargs['create_model']._id):
                raise HTTPException(
...
class File:
    ...
    class FileModel(
            Table['FileModel', FileTypes.id, AdminCreate, AdminUpdate],
            IdBase,
            table=True):
        ...
        gallery_id: FileTypes.gallery_id = Field(
            index=True, foreign_key=Gallery.GalleryModel.__tablename__ + '.' + Gallery.GalleryModel._ID_COLS[0], ondelete='CASCADE')
        size: FileTypes.size = Field(nullable=True)

        gallery: 'Gallery.GalleryModel' = Relationship(back_populates='files')
        image_file_metadata: typing.Optional['ImageFileMetadata.ImageFileMetadataModel'] = Relationship(
...
class ImageVersion:
    ...
    class ImageVersionModel(
        ...
        parent: typing.Optional['ImageVersion.ImageVersionModel'] = Relationship(
            back_populates='children', sa_relationship_kwargs={'remote_side': 'ImageVersion.id'})
        children: list['ImageVersion.ImageVersionModel'] = Relationship(
            back_populates='parent')

        image_file_metadatas: list['ImageFileMetadata.ImageFileMetadataModel'] = Relationship(
...
class ImageFileMetadata:
    ...
    class IdBase(IdObject[ImageFileMetadataTypes.file_id]):
        ...
        file_id: ImageFileMetadataTypes.file_id = Field(
            primary_key=True, index=True, unique=True, const=True, foreign_key=File.FileModel.__tablename__ + '.' + File.FileModel._ID_COLS[0], ondelete='CASCADE')
    ...
    class ImageFileMetadataModel(
        ...
        version_id: ImageFileMetadataTypes.version_id = Field(
            index=True, foreign_key=ImageVersion.ImageVersionModel.__tablename__ + '.' + ImageVersion.ImageVersionModel._ID_COLS[0], ondelete='CASCADE')
        scale: ImageFileMetadataTypes.scale = Field(
            nullable=True, ge=1, le=99)

        version: 'ImageVersion.ImageVersionModel' = Relationship(
            back_populates='image_file_metadatas')
        file: 'File.FileModel' = Relationship(
